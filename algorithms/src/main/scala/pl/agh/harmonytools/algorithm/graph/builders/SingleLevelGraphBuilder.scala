...
import pl.agh.harmonytools.algorithm.graph.node.{DoubleLevelLayer, Layer, NeighbourNode, Node, NodeContent, NodeWithNestedLayer}

class SingleLevelGraphBuilder[T <: NodeContent, S <: GeneratorInput](firstContent: T, lastContent: T) {
  ...
  protected var first: Option[Node[T]]                          = None
  protected var last: Option[Node[T]]                           = None
  ...
  protected[builders] def getFirst: Node[T] = first.getOrElse(sys.error("First not defined"))

  protected[builders] def getLast: Node[T] = last.getOrElse(sys.error("Last not defined"))
...
class NestedSingleLevelGraphBuilder[T <: NodeContent with GeneratorInput, S <: NodeContent, Q <: GeneratorInput](firstContent: T, lastContent: T)
  extends SingleLevelGraphBuilder[T, Q](firstContent, lastContent) {
  override protected def generateLayers(): Unit =
    getInputs.foreach(input => pushLayer(new DoubleLevelLayer[T, S](getGenerator.generate(input).map(new NodeWithNestedLayer[T, S](_)))))

  override protected[builders] def getFirst: NodeWithNestedLayer[T, S] = first.asInstanceOf[NodeWithNestedLayer[T, S]]

  override protected[builders] def getLast: NodeWithNestedLayer[T, S] = last.asInstanceOf[NodeWithNestedLayer[T, S]]
