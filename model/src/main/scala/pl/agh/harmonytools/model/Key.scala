...
object Key {
  ...
  def apply(keySignature: String): Key = {
    Key(
      keySignature,
      keyStrPitch.getOrElse(keySignature, throw new IllegalArgumentException("Illegal keySignature: " + keySignature)),
      baseNoteFromKeySignature(keySignature)
    )
  }

  def inferKeySignature(baseNote: BaseNote.BaseNoteType, tonicPitch: Integer): String = {
    val possibleKeySignatures = pitchKeyStr.getOrElse(tonicPitch, sys.error("Illegal tonicPitch: " + tonicPitch))
      .filter(keyCandidate => baseNoteFromKeySignature(keyCandidate).equals(baseNote)).toList
  ...
  def apply(baseNote: BaseNote.BaseNoteType, tonicPitch: Integer): Key = {
    Key(
      inferKeySignature(baseNote, tonicPitch),
      tonicPitch,
      baseNote
    )
  }
